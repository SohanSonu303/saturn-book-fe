import { useState } from "react";
import { Link } from "react-router-dom";
import { CustomButton } from "../CustomButton";

export const SigninInput = ({type} : signingTypes)=>{

    const [Postdata,SetPostdata] = useState({
        username: "",
        password:""
    });

    return (
        
        <div className="h-screen flex justify-center">
            <div className="flex justify-center  flex-col w-2/5">
            
            <InputField 
              placeholder="Enter your username" label = "Username" onChange = {
                (e :any ) => {
                    SetPostdata((prev) => ({...prev, username: e.target.value}));
                }
              } />

              
            <InputField 
              placeholder="Enter your password" label = "Password" type ="password" onChange = {
                (e :any ) => {
                    SetPostdata((prev) => ({...prev, password: e.target.value}));
                } 
              } />
                
              <CustomButton label={type} signindata= {Postdata} ></CustomButton>

                <div className="text-slate-400">
                    
                     {type !== "signup" ? <Link className="pl-2 underline" to= {"/signup"}>Don't have a account? Signup</Link>
                    : <Link className="pl-2 underline" to= {"/signin"}>Already have a account? Signin</Link> }
                     
                </div>
            </div>
        </div>
    );
}

interface labelledType{
    placeholder : string;
    label : string;
    onChange : any;
    type? : string
}


interface signingTypes{
    type : "signin" | "signup"
}
function InputField({ placeholder,label, onChange, type} : labelledType){
    return (
        <div className="mt-3">
            <label className="block mb-1 text-sm font-medium text-gray-900 dark:text-white">{label}</label>
            <input onChange = {onChange} type={type || "text"} id={label} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder= {placeholder} required />
        </div>
        );
}